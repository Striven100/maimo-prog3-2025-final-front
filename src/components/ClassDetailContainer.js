...
export default function ClassDetailContainer({ id }) {
  const { fetchClassById } = useShopContext();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mounted) setState({ loading: false, error: null, item: cls });
      } catch {
        if (mounted) setState({ loading: false, error: "No se pudo cargar la clase.", item: null });
      }
    })();
  ...
  if (state.error || !state.item) return <div className="container mx-auto px-4 py-8 text-red-400">{state.error || "Clase no encontrada."}</div>;
  ...
  return (
    <main className="container mx-auto px-4 py-8 space-y-6">
      ...
      <div className="rounded-xl border border-stone-800 bg-stone-900/70 p-6 shadow-md">
        ...
        <p className="mt-3 text-xs text-stone-400">Clase • ID: {item._id}</p>
      </div>
